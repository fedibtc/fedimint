...
use fedimint_core::runtime::{Instant, sleep, spawn, timeout};

use super::HybridBitcoinRpc;

/// Releases one backend's health-refresh slot.
...
pub(super) struct Backend {
    ...
    pub(super) rpc: Arc<dyn HybridBitcoinRpc>,
...
impl Backend {
    pub(super) fn new(name: &'static str, rpc: Arc<dyn HybridBitcoinRpc>) -> Self {
        Self {
            name,
            rpc,
            state: Arc::default(),
        ...
        let mut task = spawn("hybrid-backend-health", async move {
            ...
            let _guard = loop {
                if this
                    ...
                    .is_ok()
                {
                    break RefreshGuard(this.slot.clone());
                }
                ensure!(Instant::now() < expires, "Bitcoin health probe timed out");
