...
fn legacy_peer_status(
    p2p_receiver: &watch::Receiver<Option<P2PConnectionStatus>>,
    ci_receiver: &watch::Receiver<Option<u64>>,
    session_count: u64,
) -> LegacyPeerStatus {
    let p2p_connected = p2p_receiver.borrow().is_some();
...
mod tests {
    ...
    fn legacy_peer_status_does_not_deadlock_against_a_writer() {
        ...
        let (p2p_sender, p2p_receiver) = watch::channel(None);
        ...
        let writers = [
            {
                ...
                thread::spawn(move || {
                    ...
                    while !stop.load(Relaxed) {
                        connected = !connected;
                        p2p_sender.send_replace(connected.then_some(P2PConnectionStatus {
                            conn_type: None,
                            rtt: None,
                        }));
                    }
