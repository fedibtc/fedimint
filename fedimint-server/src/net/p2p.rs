...
enum SendHalt<M> {
    ...
    /// The configured connection lifetime elapsed.
    Expired,
}
...
impl<M: Send + 'static> P2PConnectionSMCommon<M> {
    async fn transition_connected(
        ...
        let (stop_receive, receive_stopped) = tokio::sync::oneshot::channel();
            ...
            tokio::select! {
                event = async {
                    match status_updates.as_mut() {
                        Some(updates) => updates.next().await,
                        None => std::future::pending().await,
                    }
                }, if status_updates.is_some() => {
                    if event.is_some() {
                ...
                halt = &mut send_loop => {
                    if matches!(&halt, SendHalt::Replaced(_) | SendHalt::Expired) {
                        // Both halves stop at message boundaries. An accepted
                        // frame must finish before we retire a healthy connection.
                        let _ = stop_receive.send(());
                        if let Err(error) = (&mut receive_loop).await {
                            debug!(target: LOG_NET_PEER, %error, "Retired connection failed while draining an accepted frame");
                        }
                    }
                    return match halt {
                        SendHalt::Expired => Some(self.disconnect(anyhow!("Connection exceeded the maximum age"))),
                ...
                result = &mut receive_loop => {
                    return Some(self.disconnect(result.expect_err("receive loop only stops on request")));
                },
    ...
    /// connection fails or a graceful stop is requested between frames.
